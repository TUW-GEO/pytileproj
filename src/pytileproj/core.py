    ...
    if grids is not None:
        ...
        for grid in grids[1:]:
            ...
            if not correct_order:
                raise ValueError(
                    "The given grids need to be in order, from low to high tiling levels."
                )
...
class RegularProjGridSystem(ProjGridSystemBase):
    ...
    def __init__(self, **kwargs):
        ...
        self._tms = TileMatrixSet(
            crs={"wkt": wkt}, tileMatrices=[grid._tm for grid in self.grids]
        )
    ...
    def search_tiles_in_lonlat_bbox(
        ...
        for tile in self._search_tiles_in_bbox_geog(bbox, tiling_level):
            ...
            proj_tile = self._to_proj_tile(tile, name=tilename)
            if self.tiles_in_zone_only:
                if not transform_geom_to_geog(proj_tile.boundary_ogr).Intersect(
                    self._proj_zone
                ):
                    continue
