...
def irreg_grid():
    ...
    return IrregularGrid(name="grid", tiles=tiles)
...
def test_iter_reg_tiles(reg_grid: RegularGrid):
    test_tile = None
...
def test_to_ogc(reg_grid: RegularGrid):
...
def test_irreg_grid(irreg_grid: IrregularGrid):
    assert irreg_grid.tile_ids == ["0", "3", "2", "1"]


def test_adj_matrix(irreg_grid: IrregularGrid):
    adj_matrix = np.array(
...
def test_neighbours(irreg_grid: IrregularGrid):
...
def test_iter_irreg_tiles(irreg_grid: IrregularGrid):
    test_tile = None
...
def test_irreg_tiles_bbox(irreg_grid: IrregularGrid):
