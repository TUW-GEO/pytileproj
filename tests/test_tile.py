...
@pytest.fixture(scope="session")
def ref_proj_tile(ref_extent: tuple, epsg: int, pixel_size: float) -> ProjTile:
    return ProjTile.from_extent(ref_extent, epsg, pixel_size, pixel_size)
...
def test_from_extent(ref_proj_tile: ProjTile, ref_extent: tuple):
...
def test_from_geom(ref_boundary: ogr.Geometry, pixel_size: float):
    proj_tile = ProjTile.from_geometry(ref_boundary, pixel_size, pixel_size)
...
def test_is_axis_parallel(ref_proj_tile: ProjTile):
    assert ref_proj_tile.is_axis_parallel


def test_pixel_size(ref_proj_tile: ProjTile):
    assert ref_proj_tile.x_pixel_size == ref_proj_tile.h_pixel_size
    assert ref_proj_tile.y_pixel_size == ref_proj_tile.v_pixel_size


def test_size(ref_proj_tile: ProjTile, ref_extent: tuple, pixel_size: float):
...
def test_x_coords(ref_proj_tile: ProjTile):
...
def test_y_coords(ref_proj_tile: ProjTile):
...
def test_xy_coords(ref_proj_tile: ProjTile, pixel_size: float):
...
def test_touches(
    ref_proj_tile: ProjTile, ref_extent: tuple, epsg: int, pixel_size: float
):
    ...
    raster_geom_tchs = ProjTile.from_extent(extent_tchs, epsg, pixel_size, pixel_size)
...
def test_plot(ref_proj_tile: ProjTile):
    if "matplotlib" in sys.modules:
        ...
        proj_tile = ProjTile.from_extent(
            extent, 3857, x_pixel_size=500, y_pixel_size=500
        )
        proj_tile.plot(add_country_borders=True)


def test_equal(ref_proj_tile: ProjTile, ref_boundary: ogr.Geometry, pixel_size: float):
    other_proj_tile = ProjTile.from_geometry(ref_boundary, pixel_size, pixel_size)
...
def test_not_equal(
    ...
    other_proj_tile = ProjTile.from_extent(other_extent, epsg, pixel_size, pixel_size)

    assert ref_proj_tile != other_proj_tile


def test_in(ref_proj_tile: ProjTile, ref_extent: tuple, epsg: int, pixel_size: float):
    inside_extent = (
    ...
    inside_proj_tile = ProjTile.from_extent(inside_extent, epsg, pixel_size, pixel_size)
    ...
    outside_proj_tile = ProjTile.from_extent(
